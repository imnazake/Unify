...
bool UUnifyGameplayAbility_Interaction::CanExecuteInteraction() const
{
	...
	const float MaxRange = InteractionComponent->GetScanRange() + InteractionComponent->GetScanRangeTolerance();
	...
	if (bValidateRemoteClientCameraOrigin &&
		FVector::DistSquared(CamLocation, Avatar->GetActorLocation()) > FMath::Square(InteractionComponent->GetScanRangeTolerance()))
	{
		return false;
	}
